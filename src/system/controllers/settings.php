...
switch ($action) {
    ...
    case 'vendors':
        ...
        $vendors = ORM::for_table('tbl_vendors')->find_many();
        $ui->assign('vendors', $vendors);
        $ui->display('admin/settings/vendors.tpl');
        break;
    ...
    case 'vendors-toggle':
        ...
        $id = $routes['2'];
        $vendor = ORM::for_table('tbl_vendors')->find_one($id);
        if($vendor){
            $vendor->enabled = !$vendor->enabled;
            $vendor->save();
        }
        r2(getUrl('settings/vendors'), 's', 'Vendor status updated');
        break;
