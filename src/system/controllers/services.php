...
switch ($action) {
    ...
    case 'hotspot':
        ...
        $devices = [];
        $enabledVendors = array_column(ORM::for_table('tbl_vendors')->where('enabled', 1)->find_array(), null, 'name');
        $files = scandir($DEVICE_PATH);
        foreach ($files as $file) {
            $ext = pathinfo($file, PATHINFO_EXTENSION);
            if ($ext == 'php') {
                $deviceName = pathinfo($file, PATHINFO_FILENAME);
                preg_match('/^([A-Z][a-z]+)/', $deviceName, $matches);
                $vendorName = isset($matches[1]) ? $matches[1] : '';
                if (array_key_exists($vendorName, $enabledVendors)) {
                    $devices[] = $deviceName;
                }
            }
        }
